...
public class CompPlayer implements Player {
	...
	public CompPlayer(int ident, Scanner in, PrintWriter out) {
		this.identifier = ident;
		this.board = new Board(Game.SIZE, Game.SIZE, out);
	...
	public Location placeTile(boolean retry) {
		...
		// this.output.println("Comp Board \n" + this.board.toString());
		this.output.println("Thinking...");
		// Opening moves first
		if (this.getBoardCount() == 4) {
			...
			for (int i = 0; i < Game.SIZE; i++) {
				for (int j = 0; j < Game.SIZE; j++) {
					...
					if (this.isValid(attempt, this.identifier)) {
						...
						this.executeMove(this.simBoard2, attempt, this.identifier, 0);
			...
			if (maxStableMove.size() == 1) {
				...
				for (int i = 0; i < maxStableMove.size(); i++) {
					...
					if ((hor == 0 && vert == 0) || (hor == 0 && vert == 7)
							|| (hor == 7 && vert == 0) || (hor == 7 && vert == 7)) {
						count = this
								.executeMove(this.board, test, this.identifier, 1);
			...
			if (transferForward.size() != 0) {
				...
				for (int i = 0; i < transferForward.size(); i++) {
					...
					count = this.executeMove(this.board, test, this.identifier, 1);
					...
					if (board.getTile(hor + i, vert + j).getColor() == oppID) {
						...
						while ((k < Game.SIZE) && (!found) && (!end)) {
							...
							if (((hor + k * i) < Game.SIZE) && ((hor + k * i) >= 0)
								...
								// check for piece of own color
								if (board.getTile(hor + k * i, vert + k * j).getColor() == identifier) {
									...
									for (int l = 1; l < k; l++) {
										...
										if (testCode == 0) {
											board.getTile(hor + l * i, vert + l * j)
													.setColor(identifier);
										} else if (testCode == 1) {
											count++;
										}
